...
public class RewindManager : MonoBehaviour
{
    ...
    /// </summary>
    public static Action<float, RewindManager> RewindTimeCall { get; set; }
    /// <summary>
    /// Action is not meant to be used by users. It shares data between classes. You probably want to use prepared methods like: RewindTimeBySeconds(), StartRewindTimeBySeconds(), SetTimeSecondsInRewind(), StopRewindTimeBySeconds()
    /// </summary>
    public static Action<bool, RewindManager> TrackingStateCall { get; set; }
    /// <summary>
    /// Action is not meant to be used by users. It shares data between classes. You probably want to use prepared methods like: RewindTimeBySeconds(), StartRewindTimeBySeconds(), SetTimeSecondsInRewind(), StopRewindTimeBySeconds()
    /// </summary>
    ...
    private void Update()
    {
        //print(IsBeingRewinded + this.transform.name);
    }
    ...
    public void RewindTimeBySeconds(float seconds)
    {
        ...
        TrackingStateCall?.Invoke(false, this);
        RewindTimeCall?.Invoke(seconds, this);
        RestoreBuffers?.Invoke(seconds);
        TrackingStateCall?.Invoke(true, this);
    }
    /// <summary>
    ...
    private void FixedUpdate()
    {
        if (IsBeingRewinded)
        {
            RewindTimeCall?.Invoke(rewindSeconds, this);
        }
